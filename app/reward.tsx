  ...
  return (
    <JourneyScreen dark scroll={false}>
      <View style={styles.page}>
        <Text style={styles.spark}>✦    ·    ✧</Text><Text style={styles.title}>¡Correcto!</Text>
        <Text style={styles.fact}>{mission.fact}</Text>
        <View style={styles.medallion}><Text style={styles.medallionIcon}>🪢</Text></View>
        ...
        <PrimaryButton secondary label="Siguiente" onPress={() => router.replace('/world/codigo-secreto')} />
      </View>
    </JourneyScreen>
  );
}
...
const styles = StyleSheet.create({
  ...
  medallion: { width: 190, height: 190, borderRadius: 95, alignSelf: 'center', marginVertical: 35, backgroundColor: '#071723', borderWidth: 2, borderColor: Colors.gold, alignItems: 'center', justifyContent: 'center', shadowColor: Colors.gold, shadowOpacity: .7, shadowRadius: 20 }, medallionIcon: { fontSize: 88 }, xp: { color: Colors.orange, fontSize: 25, fontWeight: '800', textAlign: 'center' }, copy: { color: Colors.white, fontSize: 13, textAlign: 'center', lineHeight: 19, marginTop: 6 }, star: { color: Colors.gold, fontSize: 37, textAlign: 'center', marginVertical: 18 },
});
